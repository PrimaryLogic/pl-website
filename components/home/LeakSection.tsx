...
import { leak } from "@/lib/content/home";
...
export default function LeakSection() {
  return (
    <Section id="leak" className="sm:!py-6 lg:!py-10">
      <div className="grid gap-x-14 gap-y-10 md:grid-cols-[minmax(240px,.68fr)_minmax(0,1.32fr)] md:items-center">
        <div>
          <Eyebrow tone="loss">{leak.eyebrow}</Eyebrow>
          <Heading className="max-w-[430px] !text-[30px] lg:!text-[38px]">{leak.heading}</Heading>
          <p className="mt-5 max-w-[430px] text-[14px] leading-[1.7] text-body">{leak.intro}</p>
        </div>

        <div>
          <ol aria-label="Illustrative stalled jobs" className="grid gap-4">
            {leak.rows.map((row) => (
              <li
                key={row.name}
                ...
                <div className="rounded-2xl border border-rule bg-white p-4 shadow-[0_8px_24px_rgba(18,20,16,0.04)]">
                  <div className="flex items-start justify-between gap-3">
                    <p className="flex items-center gap-2 text-[15px] font-semibold text-ink">
                      <Briefcase aria-hidden="true" size={20} className="text-mute" />
                      {row.name}
                    </p>
          ...
          <p className="mt-3 text-[11px] text-mute">{leak.footnote}</p>
        </div>
      </div>
    </Section>
  );
